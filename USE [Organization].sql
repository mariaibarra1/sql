USE [Organization]
GO
/****** Object:  StoredProcedure [dbo].[SyncObjects_AdmintoBranch]    Script Date: 4/7/2022 8:52:55 AM ******/
SET ANSI_NULLS ON
GO
	...
	AS
	 ...
	 -- INSERT INTO @Branches values(365,'10.0.2.25')
			...
			INSERT INTO @Objects
				   ...
				   WHERE Sc.isActive = 1 and Sc.IsBranchToAdmin = 0  and Sc.CentalDatabaseName = 'Organization'
				...
				BEGIN
					...
					WHILE @countObjects > 0
						...
						select @tableName
							...
							DECLARE @createTable NVARCHAR(MAX)= 'EXEC (''CREATE TABLE ' + @databaseName + '.dbo.SourceObjectSyncOrg(' + @columnsListDataType + ')'') AT [' + @branchIp + ']'; 
						select @createTable
							  EXECUTE (@createTable);
								...
								IF @diferentColumns = 0
								  ...
								  SET @columnsList = STUFF(
									...
									DECLARE @selectSource NVARCHAR(MAX)= '''SELECT ' + @columnsList + ' FROM ' + @databaseName + '.dbo.SourceObjectSyncOrg''';
								   ...
								   IF(@byBranch = 1)
										...
										-- Select * from GoMartBranch.dbo.SourceObjectSyncOrg
										end;
										else 
										begin
											  SET @columnsList = STUFF(
									...
									DECLARE @querySelectSource NVARCHAR(MAX)= 'SELECT ' + @ColumnsList + ' FROM ' + @databaseName + '.dbo.SourceObjectSyncOrg';
						...
						-- Select * from GoMartBranch.dbo.SourceObjectSyncOrg
									BEGIN TRY
										SET @sqlQuery = '
															EXEC(''
																...
																DROP TABLE ' + @databaseName + '.dbo.SourceObjectSyncOrg
								  ...
								  EXECUTE (@sqlQuery);
									...
									BEGIN CATCH
										...
										SET @sqlQuery = 'EXEC(''DROP TABLE ' + @databaseName + '.dbo.SourceObjectSyncOrg'') AT [' + @branchIp + ']';
										EXECUTE (@sqlQuery);
									END CATCH;
								END TRY
					...
					SET @countBranch-=1;
					select @maximo
				  -- UPDATE  [Sync].[dbo].[LastSync] SET [SyncPendingId] = @maximo	WHERE branchId = @paramBranchId; 
				   --delete from  [Sync].[Logs].[SyncPending];
